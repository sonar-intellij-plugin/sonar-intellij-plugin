...
public class SonarServer {
  ...
  private Sonar createSonar() {
    ...
    if (mySonarServerConfig.isAnonymous()) {
      ...
      newSonar = createSonar(
        mySonarServerConfig.getHostUrl(),
        mySonarServerConfig.getUser(),
        mySonarServerConfig.getPassword()
      );
  ...
  private Sonar createSonar(String host,String user,String password) {
    ...
    return StringUtils.isEmpty(user)
      ? Sonar.create(safeHost)
      : Sonar.create(safeHost,user,password);
  }
  ...
  //  https://sonar.corp.mobile.de/sonar/api/resources?format=json&resource=autoact:autoact-b2b-api_groovy&metrics
  // =profile
